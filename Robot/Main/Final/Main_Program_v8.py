...
def keepDistance():
    ...
    while distanceThread:
        ...
        if sign == 1:
            outputSpeed = int(np.interp(pid_speed, [0, 500], [speed_min_robot, speed_max_robot]))
        else:
            outputSpeed = int(np.interp(pid_speed, [-100, 0], [-abs(speed_max_robot), -speed_min_robot]))
