...
def OdoThread():
    ...
    pose, cov = get_motor_state(odo_pose, motor_ticks, odo_cov)
...
def KalmarFilter():
    ...
    global odo_pose, odo_cov, odo_pose_flip
    ...
    while True:
        try:
            ...
            odo_pose, odo_cov = odo_thread.join()
            # updates the flip value
            odo_pose_flip = [-(odo_pose[1]), odo_pose[0], np.rad2deg(odo_pose[2])]
            # wait for 10 ms
            time.sleep(0.02)
