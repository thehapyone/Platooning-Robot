        ...
        for detection in detections:
            ...
            if detection.ClassID == 1:
                ...
                if object_area < 15000:
                    newTop = abs(int((object_bottom - object_top) / 2))
                    newTop = newTop + object_top
                    #cv.rectangle(aimg1,(object_left,object_top),(object_right,object_bottom),(200,0,200),3)
                    # roi
                    roi = aimg2[newTop:object_bottom, object_left:object_right]
                    #new_image = cv.resize(roi)
                    # convert to grascale
                    gauss_image = cv.GaussianBlur(roi, (3,3), 0)
                    # convert to hsv
                    hsv_image = cv.cvtColor(gauss_image, cv.COLOR_BGR2HSV)
                    # define a range of blue color in HSV
                    lower_blue = np.array([75, 200, 200])
                    upper_blue = np.array([169, 255, 255])

                    # apply threshold to the HSV image to get only blue colors.
                    mask = cv.inRange(hsv_image, lower_blue, upper_blue)
                    rectKernel = cv.getStructuringElement(cv.MORPH_RECT,(7,7))
                    dilation = cv.dilate(mask,  rectKernel, iterations = 2)
                    # check if image has 1
                    light_state = np.all(dilation==0)

                    if light_state == False:
                        # means it detect green light or yellow
                        #insect_detections.append(
                        # send in 1 id detection is true
                        insect_results.light_state = 1
                    else:
                        insect_results.light_state = 0
                        
                    cv.imshow('segment no:',dilation)
